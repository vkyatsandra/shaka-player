  ...
  parseMedia(data, time, uri) {
    ...
    const parser = new Mp4Parser()
        ...
        .fullBox('tfdt', (box) => {
          ...
          sawTFDT = true;
          if (this.isTTMLBox_ && this.timescale_ > 0) {
    ...
    if (sawTFDT && this.isTTMLBox_ && this.timescale_ > 0) {
      const fullData = shaka.util.Uint8ArrayUtils.concat(...mdats);
      payload = this.parser_.parseMedia(fullData, time, uri, /* images= */ []);
    } else if (sawTRUN) {
      const fullData =
