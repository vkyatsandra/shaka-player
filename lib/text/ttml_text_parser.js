...
shaka.text.TtmlTextParser = class {
  ...
  parseCue_(
    ...
    // For HLS, TTML XML times are segment-relative (0-based within the
    // segment), so we add segmentStart to get absolute presentation times.
    // For DASH, TTML XML times are already period-relative or absolute, so
    // we add periodStart (typically 0) — matching the original behaviour.
    const isSegmentRelative =
        this.manifestType_ === shaka.media.ManifestParser.HLS &&
        timeContext.segmentStart !== undefined &&
        timeContext.segmentEnd !== undefined &&
        timeContext.segmentStart >= 0;
    if (isSegmentRelative) {
      // HLS: times in XML are 0-based; anchor to segment start.
      start += timeContext.segmentStart;
    } else {
      // DASH / standalone TTML: times are period-relative or absolute.
    ...
    if (end == null) {
      ...
      if (isSegmentRelative) {
        end += timeContext.segmentStart;
      } else {
        end += timeContext.periodStart;
      }
    }

    if (timeContext.segmentStart !== undefined &&
        timeContext.segmentEnd !== undefined &&
        timeContext.segmentEnd !== Infinity) {
